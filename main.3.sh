...
#{{{###   Dependencies                                 ###
##########################################################
DEPENDS=( 'ufw' 'git' 'samba' 'minidlna' 'pass' 'isync' 'abook' 'neomutt' )
...
# Loop through the list of dependencies and check each one
...
if [ ${#missing_packages[@]} -eq 0 ]; then
    ...
    $execute_command "${missing_packages[@]}"
fi
#}}}
##########################################################
#{{{###   IF BATAN                                     ###
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
	if [ -f /etc/os-release ]; then
		...
		OS=$NAME
		VER=$VERSION_ID
		NK1="\033[36m[\033[31mx\033[36m]\033[0m"
...
NK2="\033[36m[\033[31mx\033[36m]\033[0m"
...
then
	cat /etc/fstab >> fstab
echo "UUID=c96173e2-aae6-43b1-bad3-2d8fb4e87e25 /media/batan/100/ ext4 defaults,rw 0 0" >> fstab
sudo mv /etc/fstab.$(date +%j).org
...
NK3="\033[36m[\033[31mx\033[36m]\033[0m"
################################################
#}}}
##########################################################
#{{{###   Setup SSH-keys & Firewall                    ###
##########################################################
...
sudo ufw enable
NK4="\033[36m[\033[31mx\033[36m]\033[0m"
...
NK5="\033[36m[\033[31mx\033[36m]\033[0m"
...
# Variables
USER="batan"
MEDIA_DIR="/media/batan/100"
HOME_DIR="/home/$USER"
MEDIA_DIRS=("$MEDIA_DIR/Videos" "$MEDIA_DIR/Music" "$MEDIA_DIR/Pictures")
...
echo -e "\033[37mMiniDLNA setup complete. Media should now be available on the local network."
NK6="\033[36m[\033[31mx\033[36m]\033[0m"
#}}}
...
##########################################################
#!/bin/bash
# 2. Install i3 and required dependencies
echo "Installing dependencies and i3 desktop environment..."
sudo apt install -y i3 lightdm surf stterm lightdm-gtk-greeter xorg rofi feh compton sddm picom thunar
# 3. Enable lightdm service to start at boot
echo "Enabling lightdm..."
sudo service lightdm enable
# 4. Set up i3 configuration files (optional but helpful)
echo "Setting up default i3 configuration..."
mkdir -p ~/.config/i3
cp /etc/i3/config ~/.config/i3/config  # Example of copying a default config
# You can customize the config if needed here
# 5. Cleanup
echo "Cleaning up unnecessary packages..."
sudo apt autoremove -y
sudo apt clean
echo "Setup complete! Reboot to start i3 desktop environment."


#}}}
##########################################################
#{{{###  CUSTOM KERNEL  [HARDEND or ZEN]               ###
##########################################################
curl -s 'https://liquorix.net/install-liquorix.sh' | sudo bash
